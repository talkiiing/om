...
const GlowButton = (props: IButtonProps) => {
  return (
    <button
      className={`
        ...
        !props.type || props.type === 'solid'
          ...
          : props.type === 'outline'
          ? 'hover:text-omblue-400'
          : 'ring-0'
      } w-full ring-2 ring-omblue-500 hover:ring-omblue-600 active:ring-omblue-700 focus:ring-omblue-700 
       px-4 py-2 shadow-md rounded-lg hover:shadow-sm text-gray-200
    ...
    >
      <div className='flex flex-row justify-center space-x-2 transform-colors duration-75'>
        {props.icon && <div>{props.icon()}</div>}
        {props.value && (
          <div className='whitespace-nowrap'>{props.value || ''}</div>
        )}
      </div>
    </button>
  )
